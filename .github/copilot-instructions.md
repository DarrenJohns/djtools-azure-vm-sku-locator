...
A second, experimental Top Trumps build **`toptrumps-beta.html`** also ships from site root. It renders the cards and space scene directly in Three.js, including animated moons, a galaxy backdrop, rarity foil, and X-wing/TIE fighter flybys. It loads Three.js r170, `GLTFLoader`, textures, and models from the committed `vendor/` and `assets/` directories via a native browser import map (no CDN, no build step, no npm). The older `toptrumps-webgl.html` and `toptrumps-pure.html` files remain development artifacts and are not deployed.
...
- **No frameworks**: Pure vanilla HTML/CSS/JS — no React, Vue, npm, etc. **Exception**: `toptrumps-beta.html` loads Three.js r170 and `GLTFLoader` from `vendor/` via a native browser import map. All runtime assets are committed to the repo; nothing is fetched from a CDN.
