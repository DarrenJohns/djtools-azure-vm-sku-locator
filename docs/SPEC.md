...
- **Match HUD**: a centred scoreboard mirrors the playfield ownership (`CPU` left, `PLAYER` right), with circular outer card counters, an overlapping `VS` medallion, and an attached gameplay-notification tray. Messages remain centred when they fit and scroll right-to-left only when they overflow. The HUD is hidden on the title/how-to screens.
