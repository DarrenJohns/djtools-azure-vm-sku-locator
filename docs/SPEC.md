...
- **Match HUD**: a centred split-wing scoreboard mirrors the playfield ownership (`CPU` left, `PLAYER` right). Equal team panels carry each side's live card count and meet at a compact hexagonal `VS` node. It is hidden on the title/how-to screens.
