...
5. **Centred match scoreboard** — a DOM HUD uses equal split wings for CPU/card count on the left and PLAYER/card count on the right, joined by a compact hexagonal `VS` node. `updateHUD()` refreshes the counts while a match-visibility helper keeps the scoreboard off the title and how-to screens.
